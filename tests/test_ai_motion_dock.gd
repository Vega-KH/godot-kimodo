...
func _run() -> void:
	for cycle in 3:
		...
		var save_action: Button = dock.find_child("SaveSelectedTake", true, false)
		var save_type: OptionButton = dock.find_child("SaveTakeType", true, false)
		...
		_check(workspace.get_tab_count() == 2, "active workspace has focused Generate and Preview & Save tabs")
